...
def main():
    ...
    for seed in range(args["runs"]):
        ...
        for named, param in n.named_parameters():
            mean = torch.sum(grads[counter])
            dif += torch.abs(n.grads[named] - grads[counter]).sum()
            positive = ((n.grads[named] * grads[counter]) > 1e-4).float().sum()
            total = positive + ((n.grads[named] * grads[counter]) < - 1e-4).float().sum()
            total_sum += total
            mean_val += mean
        ...
        logger.debug("Meanw")
        logger.debug("Total %s", total_sum)
